...
export const CurrentWeather = ({ data, city }: { data: List; city: City }) => {
  ...
  const { description, id } = current;
  const { temp, temp_max, temp_min } = data.main;
  const dayName = useDayName(city.timezone, data.dt, "long");
      ...
      <CardContent>
        <div className="py-12 flex flex-col gap-4">
          <div className="text-9xl font-bold text-center">
            <span>{formatTemperature(temp, units)}</span>
          </div>
        </div>
      </CardContent>
      ...
      <CardFooter>
        <div className="flex flex-1 justify-between items-end">
          ...
          <div className="flex gap-2 opacity-70">
            <span>H: {formatTemperature(temp_max, units)}</span>
            <span>L: {formatTemperature(temp_min, units)}</span>
          </div>
        </div>
      </CardFooter>
    </Card>
